...
int text_newline(TText *t, unsigned pos, unsigned split);
...
int text_newline(TText *t, unsigned pos, unsigned split)
{
	...
	if(pos<t->nlines) {
		...
		if( split < t->lines[pos-1]->len ) {
			memcpy(
				newl->contents,
				&t->lines[pos-1]->contents[split],
				t->lines[pos-1]->len - split
			);
			newl->len = t->lines[pos-1]->len - split;
			t->lines[pos-1]->len=split;
		}
	}
	t->lines[pos]=newl;
...
int text_remline(TText *t, unsigned pos)
{
	if(pos>=t->nlines)
		return 0;
	if(t->nlines < 1)
		return 0;
	if( pos > 0 && t->lines[pos]->len + t->lines[pos-1]->len > 255 )
		return 0;
	if(t->lines[pos]){
		if(pos>0 && t->lines[pos]->len ) {
			unsigned lb = t->lines[pos-1]->len;
			memcpy(
				&(t->lines[pos-1]->contents[lb]),
				t->lines[pos]->contents,
				t->lines[pos]->len
			);
			t->lines[pos-1]->len += t->lines[pos]->len;
		}
		free(t->lines[pos]);
		t->lines[pos]=0;
	}
	if( t->nlines - pos > 1)
	{
		memmove( &t->lines[pos], &t->lines[pos+1], (t->nlines-pos-1)*sizeof(TLine *) );
	}
	t->nlines--;
	return 1;
}
...
int text_ctrl_newline(TControl *tc)
{
	if(text_newline(&tc->text, tc->cy, tc->cx)) {
		tc->cx=0;
...
int text_ctrl_edit(TControl *tc, unsigned char ch)
{
	...
	if(tc->cy>=tc->text.nlines)
	{
		if(!text_newline(&tc->text, tc->cy,0)) {
			return 0;
		}
	}
...
int text_ctrl_delete(TControl *tc, unsigned char ch)
{
	...
	if(tc->cx==0 && tc->cy > 0 )
	{
		unsigned ncx = tc->text.lines[tc->cy-1]->len;
		if(text_remline(&tc->text, tc->cy)) {
			tc->cx=ncx;
			tc->cy--;
		}
		text_ctrl_draw(tc);
